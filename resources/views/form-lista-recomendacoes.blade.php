...
@section('content')
 ...
 <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Cadastrar Lista de Recomendações Técnicas
        </h2>
         <a href="{{ route('show-lista-recomendacoes') }}" class="btn btn-primary shadow-md mr-2"><i data-feather="skip-back" class="w-4 h-4 mr-2"></i>Voltar</a>
    </div>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="col-span-12 lg:col-span-12 xxl:col-span-12 tab-content">
            ...
            @endforeach
            <form action="{{route('cad-lista-recomendacoes')}}" method="post" enctype="multipart/form-data" data-single="true"
                method="post">
                @csrf
                    ...
                    <div class="intro-y box col-span-12 xxl:col-span-12 p-5">
                        <div class="grid grid-cols-12 gap-x-5">
                            <div class="col-span-12 xl:col-span-6">
                                <div class="mt-3">
                                    <label for="nome" class="form-label"><strong>Recomendação:</strong></label>
                                     <input id="num_funcionarios" type="text" name="recomendacao[]" class="form-control"
                                        placeholder="Recomendação Técnica"  required>
                        ...
                        <button type="button" class="btn btn-primary w-40" style="margin:35px" onclick="addDados()">Adicionar Mais Técnicas</button>
                            <button type="submit" class="btn btn-primary w-full  mr-2 mb-2"> <i data-feather="activity"
                                    class="w-4 h-4 mr-2"></i>
                                Cadastrar Lista de Recomendações Técnicas</button>
                        </div>
                    </div>
                    </div>
            </form>
...
function removeInput(element) {
    ...
    function addDados() {
        ...
        inputField.innerHTML = `
            <div class="mt-3">
                <label for="nome" class="form-label"><strong>Recomendação:</strong></label>
                <input id="num_funcionarios" type="text" name="recomendacao[]" class="form-control" placeholder="Recomendação Técnica" required>
                                        <span class="input-group-append close-icon close-button" onclick="removeInput(this)" >
                                            <i data-feather="x-circle" class="w-4 h-4 mr-2"></i>
                                        </span>
            </div>
